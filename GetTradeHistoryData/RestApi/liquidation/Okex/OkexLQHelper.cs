...
namespace GetTradeHistoryData
{
    public class OkexLQHelper
    {
        ...
        private void GetLQdata(string symbol, string key, string type, string ccy, string alias, string instId, instruments item)
        {
            ...
            //获取初始数据,获取页数数据
            OkexLq result = GetResultData(type, contractCode, timestamp, alias);
            ...
            if (result.details.Count() > 0 && result.details.Count ==100)
            {
                ...
                if (orderslist.Count > 0)
                {
                    if (orderslist.Where(p => p.ts > timestamp).Count() > 0)//是否存在大于最后一次的时间戳
                    {
                        ...
                        while (true)
                        {
                            var results = GetResultData(type, contractCode, maxtime, alias);
                            ...
                            Console.WriteLine("okex--"+symbol + GZipDecompresser.GetTimeFromUnixTimestampthree(maxtime.ToString()).ToString());
        ...
        private OkexLq GetResultData(string type, string contractCode, long before, string alias)
        {
            string location = "";
            if (alias == "")
            {
        ...
        private void GetFundingRateAndOpenInterest(string type)
        {
           ...
           string messagetype = type == "SWAP" ? "永续费率和持仓" : "交割费率和持仓";
            ...
            foreach (var item in openlist)
            {
                ...
                var vol = ticklist.FirstOrDefault(p => p.instId == item.instId);
                ...
                if (sy != null && mp != null)
                {
                    ...
                    if (vol != null)
                    {
                        o.volumeUsd24h = vol.last * vol.volCcy24h;  //价格*币
                    }
            ...
            Console.WriteLine("okex 结束持仓和费率数据获取：" + CommandEnum.RedisKey.okex + DateTime.Now.ToString()+ messagetype);
        }
    }
}
